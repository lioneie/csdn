...
读文件时，如果有数据需要写到server端，而这时又刚好有个致命错误发生，`page->mapping`被置为`NULL`，接着执行到`nfs_readpage_async`就发生了空指针解引用。具体根因和解决方案请查看《[4.19 nfs_updatepage()空指针解引用问题](https://chenxiaosong.com/courses/nfs/issues/4.19-null-ptr-deref-in-nfs_updatepage.html)》。
```c
read
  ksys_read
    vfs_read
...
这里我只找到办法在direct读的时候触发回写，暂时还未找到在buffer读的时候触发回写（因为buffer读直接从page cache里读取数据），有兴趣的朋友可以和我一起尝试。direct读触发回写的用户态程序是[direct-read-trigger-writeback.c](https://gitee.com/chenxiaosonggitee/blog/blob/master/src/nfs/direct-read-trigger-writeback.c)，执行以下脚本:
```sh
touch /mnt/file
sync
...
direct读没有执行到`nfs_readpage`，以下是gdb断点的栈（有关gdb调试内核的方法可以查看《[Linux内核课程](https://chenxiaosong.com/courses/kernel/kernel.html)》）:
```sh
(gdb) bt
