...
在阿里云上配置DNS域名解析，添加记录类型`A`解析主域名`@`到记录值`185.199.108.153`、`185.199.109.153`、`185.199.110.153`、`185.199.111.153`之一（多个同时加好像不能生成ssl，但我不是很确定哈），添加记录类型`A`解析`www`到和`@`一样的记录值（添加记录类型`CNAME`解析`www`到主域名好像有问题）。
