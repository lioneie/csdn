...
[点击这里查看`bt -g 227403`的输出](https://gitee.com/chenxiaosonggitee/tmp/blob/master/nfs/nfs-soft-lockup-in-__rpc_execute-crash-vmcore-bt-g-227403.txt)。
...
挂载命令如下:
```sh
mount -t nfs -o rw,relatime,vers=3,rsize=1048576,wsize=1048576,namlen=255,hard,proto=tcp,timeo=600,retrans=2,sec=sys,mountvers=3,mountproto=udp,local_lock=none,_netdev localhost:/tmp/s_test /mnt # mountport=2050
```
...
[从vmcore的解析`bt -g 227403`的输出](https://gitee.com/chenxiaosonggitee/tmp/blob/master/nfs/nfs-soft-lockup-in-__rpc_execute-crash-vmcore-bt-g-227403.txt)可以看到进程组里没有一个进程在等待异步rpc读的回复。
...
下次再复现出soft lockup后，打开调试开头收集更多的nfs和rpc的日志:
```sh
